...
if [ -z "${3:-}" ]; then
  echo "Output file not provided as 4th argument"
  exit 1
fi

# check for prefix file name

if [ -z "${4:-}" ]; then
  prefix=""
else
  queryParams="${queryParams}&prefix=${prefix}"
fi
...
echo "Fetching bucket list" "${outputFile}" "<-" "${bucket}" "${region}"
...
curl -f -s -L -o "${outputFile}" \
  --proto-redir =https -X "${httpReq}" \
  ...
  "https://${bucket}${baseUrl}/${req}?${queryParams}" || exit 1
