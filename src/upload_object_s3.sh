...
uploadFileName="${5:-example-new-name}"
...
fileRemote="${uploadFileName}"

if [ -z "${region}" ]; then
  region="${awsRegion}"
fi

echo "Uploading" "${fileLocal}" "->" "${bucket}" "${region}" "${storageClass}" "as" "${uploadFileName}"
...
curl -f -s -L --proto-redir =https -X "${httpReq}" -T "${fileLocal}" \
  ...
  "https://${bucket}${baseUrl}/${fileRemote}" || exit 1
